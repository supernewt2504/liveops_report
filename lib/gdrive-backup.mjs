...
async function findExisting(token, name) {
  ...
  // supportsAllDrives/includeItemsFromAllDrives: 공유 드라이브(Shared Drive)에서도 검색되도록
  const r = await fetch(`https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id)&spaces=drive&supportsAllDrives=true&includeItemsFromAllDrives=true`,
...
async function uploadOne(jwt, localPath, driveName, mime) {
  ...
  const url = existingId
    ? `https://www.googleapis.com/upload/drive/v3/files/${existingId}?uploadType=multipart&fields=id,name&supportsAllDrives=true`
