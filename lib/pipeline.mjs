...
export async function runPipeline({ mail = true, testTo = null, mailProject = null, subjectSuffix = null } = {}) {
  ...
  await step('2.6 고객센터 AI 요약', 'cs-summarize.mjs', {}, true);          // helpdesk 프로젝트만·키 없으면 스킵
  ...
  if (mail) { // 프로젝트별 발송 (자격증명/수신자 없으면 각 프로젝트 스킵). mailProject 지정 시 해당 프로젝트만(테스트용).
    const mailProjs = mailProject ? (cfg.projects || []).filter(p => p.id === mailProject) : (cfg.projects || []);
    for (const p of mailProjs) await step(`5 리포트 메일 발송 [${p.id}]`, 'send-mail.mjs', { PROJECT: p.id, ...(testTo ? { MAIL_TEST_TO: testTo } : {}), ...(subjectSuffix ? { MAIL_SUBJECT_SUFFIX: subjectSuffix } : {}) }, true);
  }
  console.log('\n✅ 파이프라인 완료');
}

// 일중 순위 피크만 갱신 → 대시보드 재빌드 (수집·요약·메일 없음). 하루 여러 번 호출.
export async function runRankPeak() {
  await step('순위 피크 수집', 'rank-peak.mjs', {}, true);
  const cfg = JSON.parse(readFileSync(join(ROOT, 'config.json'), 'utf8'));
  for (const p of (cfg.projects || [])) await step(`대시보드 재빌드 [${p.id}]`, 'build-dashboard.mjs', { PROJECT: p.id }, true);
  console.log('\n✅ 순위 피크 갱신 완료');
}
