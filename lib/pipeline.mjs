...
export async function runPipeline({ mail = true, testTo = null, mailProject = null } = {}) {
  ...
  if (mail) { // 프로젝트별 발송 (자격증명/수신자 없으면 각 프로젝트 스킵). mailProject 지정 시 해당 프로젝트만(테스트용).
    ...
    for (const p of mailProjs) await step(`5 리포트 메일 발송 [${p.id}]`, 'send-mail.mjs', { PROJECT: p.id, ...(testTo ? { MAIL_TEST_TO: testTo } : {}) }, true);
  }
  console.log('\n✅ 파이프라인 완료');
}
